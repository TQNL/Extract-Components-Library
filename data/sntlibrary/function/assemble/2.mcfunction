...
# check if the data is a text-based value with single quotes that get lost in the macro
...
execute if data storage get_components:extract {sign:{check:"'"}} run tellraw @p {"color":"yellow","nbt":"sign.string","storage":"get_components:extract"}
## also remove data in next function - folders
data remove storage get_components:extract sign

# try the concatination the normal way
execute if function sntlibrary:assemble/init_3 run return 1
...
function sntlibrary:assemble/3_correction with storage get_components:extract combining
